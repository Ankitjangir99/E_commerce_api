...
module.exports.create = function(req, res){
    ...
    newProduct.save(function(err){
        if(err){
            ...
            res.send(`Product is added with name ${req.body.name} and quantity ${req.body.quantity} !`);
        }
    });
}
    ...
    Product.deleteOne(
        ...
        function(err){
            if(err){
                ...
                res.send({
                    message: `Your product is deleted successfully whose id is ${req.params.productID}`
                });
            }
        });
}
            ...
            Product.findByIdAndUpdate(ID, {quantity: newQty}, function(err, updatedProduct){
                if(err){
                    ...
                    res.send({
                        ...
                        message: `yup! product is upadated with new quantity ${newQty} !!`
                    });
                }
            });
        }
